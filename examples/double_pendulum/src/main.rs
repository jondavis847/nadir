...
fn main() {
    let base = Base::new("base", multibody::base::BaseSystems::Basic(Some(Gravity::Constant(ConstantGravity::new(0.0,0.0,-9.8)))));    
    ...
    let b1 = Rc::new(RefCell::new(Body::new("b1", mp).unwrap().with_mesh(b1_mesh)));
    ...
    let b2 = Rc::new(RefCell::new(Body::new("b2", mp).unwrap().with_mesh(b2_mesh)));
    ...
    r1.borrow_mut().connect_base(&base, Transform::IDENTITY).unwrap();
    r1.borrow_mut().connect_outer_body(&b1, top.clone()).unwrap();
    b1.borrow_mut().connect_inner_joint(&r1).unwrap();
    b1.borrow_mut().connect_outer_joint(&r2).unwrap();
    r2.borrow_mut().connect_inner_body(&b1, bottom.clone()).unwrap();
    r2.borrow_mut().connect_outer_body(&b2, top.clone()).unwrap();
