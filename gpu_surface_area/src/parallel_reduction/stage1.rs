...
use crate::parallel_reduction::Uniforms;

pub struct Stage1 {
    buffers: Stage1Buffers,
...
struct Stage1Buffers {
    workgroup_id: wgpu::Buffer,
    workgroup_pos_x: wgpu::Buffer,
    workgroup_pos_y: wgpu::Buffer,
    workgroup_pos_z: wgpu::Buffer,
}
...
impl Stage1Buffers {
    fn new(device: &wgpu::Device, uniforms: &Uniforms) -> Self {
        ...
        let buffer_size = (4 * uniforms.num_workgroups * uniforms.num_objects) as u64;

        Self {
            workgroup_id: make_buf(
                "workgroup_id",
                buffer_size,
                wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
            ),
            workgroup_pos_x: make_buf(
                "workgroup_pos_x",
                buffer_size,
                wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
            ),
            workgroup_pos_y: make_buf(
                "workgroup_pos_y",
                buffer_size,
                wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
            ),
            workgroup_pos_z: make_buf(
                "workgroup_pos_z",
                buffer_size,
                wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
            ),
...
impl Stage1 {
    pub fn new(
        ...
        let buffers = Stage1Buffers::new(device, uniforms);
