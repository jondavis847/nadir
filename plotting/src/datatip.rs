...
impl Datatip {
    pub fn new(anchor: Point, color: usize, x_value: f64, y_value: f64) -> Self {
        let bounds = Rectangle::new(
            anchor + Vector::new(30.0, -30.0),
            Size::new(50.0, 25.0),
        );
        Self { anchor, bounds, color, text_size: 9.0, x_value, y_value }
    }
    pub fn draw(&self, frame: &mut Frame, theme: &PlotTheme, lines: &Vec<LineHandle>) {
        let line = lines[self.color]
            .lock()
            .unwrap();
        ...
        let stroke = Stroke { style: Style::Solid(line_color), ..Default::default() };
        let path = Path::line(
            self.anchor,
            self.bounds
                .center(),
